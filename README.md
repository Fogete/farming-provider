...
`WantStrategy` will be added to the vault with:

```python
vault.addStrategy(wantStrategy, x, 0, 10k in usd, 1_000)
```

and after the first harvest, it will be closed down with:

```python
vault.updateStrategyMaxDebtPerHarvest(wantStrategy, 0)
```

Harvest will check for want balance and if there is any, it will be sent to `Joint`

Once the investment is done (aka capital deposited into the joint), the strategy is removed from the withdraw queue and sets a `takeProfit` flag.

The `takeProfit` flag is used in `harvest()` to decide if want should go to `Joint` or return as profit to the vault.
...
Envision the `Joint` contract as a strategist dashboard.

### CreateLP

Who can call: `onlyStrategist`
This function takes care of creating an LP token with balance of A, B.

### SetProviderA() and setProviderB()

Who can call: `onlyGov`
This two variables are going to be used to distribute profit. Only gov can call since there is where profit goes.

### LiquidatePosition

Who can call: `onlyStrategist`
Undo the LP and return A funds to providerA and B funds to providerB.

### Harvest

Who can call: `onlyStrategist`

Harvest will work very similar to yearn-vaults harvest.
The only tweak is adding a flag to decide if we should reinvest (on by default) or distribute profit.

```solidity

// Convert reward to A and B on equal value
swapReward();
...
`distributeProfit` just sends tokens back to the provider strategies.

## Worst case scenario

Worst case scenario, we lose the equal investment and one of the provider will report a loss.
